...
<script>
	export default {
		...
		methods: {
			...
			//当选择项发生变化时会触发该事件，即勾选了行或取消勾选，传入选中的这一行的所有数据
			...
			//搜索方法，当我们在搜索框输入，比如admin,那就会把input绑定的searchForm.username当作参数传回去
			//比如现在我在input打admin，点击搜索时，get请求就是Request URL: https://okr1-api.foxconnedu.com/sys/user/list?username=test&current=1&size=10
			//不太理解为什么要传current&size，先不考虑这个，https://okr1-api.foxconnedu.com/sys/user/list?username=test&current=1&size=10
			getUserList() {
				this.$axios.get("/sys/user/list", {
					...
					console.log(res,'我是getuserlist')
					this.tableData = res.data.data.records
					//size是指默认一页有几条数据
					this.size = res.data.data.size
					//current是指当前是表格的第几页，默认第一页
